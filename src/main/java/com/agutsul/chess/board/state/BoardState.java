...
public interface BoardState
    ...
    enum Type {
        ...
        CHECKED(2, false),
        CHECK_MATED(1, true),
        STALE_MATED(3, true),            // draw - when the player to move is not in check but has no legal move ( automatic )
        THREE_FOLD_REPETITION(7, false), // draw - when the same position occurs three times with the same player to move ( claims arbiter )
        FIVE_FOLD_REPETITION(4, true),   // draw - when the same position occurs five times with the same player to move  ( automatic )
        ...
        SEVENTY_FIVE_MOVES(5, true),     // draw - when the last seventy five successive moves made by both players contain no capture or pawn move ( automatic ). If the last move was a checkmate, the checkmate stands.
